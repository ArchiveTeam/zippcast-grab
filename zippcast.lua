...
wget.callbacks.download_child_p = function(urlpos, parent, depth, start_url_parsed, iri, verdict, reason)
  ...
  if (downloaded[url] ~= true and addedtolist[url] ~= true) and (string.match(url, item_value) or html == 0) then
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  local function check(urla)
    ...
    if (downloaded[url] ~= true and addedtolist[url] ~= true) and (string.match(url, item_value) or string.match(url, "%.jpg$")) then
...
wget.callbacks.httploop_result = function(url, err, http_stat)
  ...
  if status_code >= 500 or
    ...
    if tries >= 5 then
      ...
      io.stdout:flush()
      tries = 0
      if string.match(url["url"], "^https?://[^/]*zippcast%.com") and not (string.match(url["url"], "%.mp4$") or string.match(url["url"], "^https?://node[0-9]+%.sv[0-9]+%.zippcast%.com/files_thumbnail/")) then
        return wget.actions.ABORT
      else
        return wget.actions.EXIT
      end
